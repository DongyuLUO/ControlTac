...
第一阶段严格为 20,000 个 samples，第二阶段为 7,000 张不重复图片。每个物理物体的份额尽可能均衡，cylinder92_1 和 cylinder92_2 合计占一个物体的份额。不能整除造成的余数以固定方式分配。

**实际缺口：** cylinder92_1 完整本地数据只有 1,517 张；保留独立验证、测试姿态后剩 1,491 张。当前恢复方案固定重复采样 176 次，因此第一阶段是 **19,824 张不同图片、20,000 个 samples**。这不是新增的真实图片，不能据此宣称拥有 20,000 张 unique images。可以关闭重复采样重新生成，但此时脚本会明确报数量不足。

第二阶段六个物体各有 300 个不同姿态；cylinder92 的两个子集各 150 个，并排除相互重复的姿态。训练、验证、测试之间没有姿态交叉。验证集 911 条，测试集 957 条。原始 CSV、每条样本来源和统计见 `splits/`。
