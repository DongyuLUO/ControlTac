...
class ControlTransformerBlock(Module):
    def __init__(self, base_block: ForceTransformerBlock, block_index: int) -> None:
        super().__init__()
    ...
    def forward(self, x, y, t, control_signal, mask=None, HW=None):
        if self.block_index == 0:
            ...
            control_signal = self.copied_block(x + control_signal, y, t, (8, 10))
            ...
            control_signal = self.copied_block(control_signal, y, t, (8, 10))
            control_signal_skip = self.after_proj(control_signal)

        return control_signal, control_signal_skip

class ForcePoseControlTransformer(ForceControlTransformer):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.x_con_embedder = PatchEmbedMS(patch_size, in_channels//2, hidden_size, kernel_size=kernel_size, bias=True)

        drop_path = [x.item() for x in torch.linspace(0, drop_path, depth)]  # stochastic depth decay rule
        self.blocks = nn.ModuleList(
            [
                ForceTransformerBlock(
                    hidden_size,
                    num_heads,
                    mlp_ratio=mlp_ratio,
        ...
        self.controlnet = nn.ModuleList([ControlTransformerBlock(self.blocks[i], i) for i in range(copy_blocks_num)])
    ...
    def forward_controlnet(self, control_signal):
        if self.use_pe:
            control_signal = self.x_con_embedder(control_signal) + self.pos_embed_ms
        else:
            control_signal = self.x_con_embedder(control_signal)
        return control_signal

    def forward(self, x, timestep, control_signal, force, mask=None, data_info=None, control_strength=None, **kwargs):
        """
        ...
        """
        control_strength = self.control_scale if control_strength is None else control_strength
        ...
        t0 = self.t_block(t)
        force = self.force_embedder(force) # (N, D)
        if self.force_norm:
            force = self.attention_force_norm(force)



        # Released weights were trained with block 0 skipped. New training restores it.
        if not getattr(self, 'legacy_skip_first_block', True):
            x = self.blocks[0](x, force, t0, (self.h, self.w), **kwargs)
        for i in range(1, self.copy_blocks_num + 1):
            control_signal, control_signal_skip = self.controlnet[i - 1]( x, force, t0, control_signal, (self.h, self.w), **kwargs)
            x = self.blocks[i](x + control_signal_skip * control_strength, force, t0, (self.h, self.w), **kwargs)

        for i in range(self.copy_blocks_num + 1, len(self.blocks)):
            x = self.blocks[i](x, force, t0, (self.h, self.w), **kwargs)
