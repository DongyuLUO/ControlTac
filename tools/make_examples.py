...
for stage, filename, checkpoint in [('force','force.json','force_control.pth'),('force_pose','force_pose.json','force_pose_control.pth')]:
    example = dict(stage=stage, checkpoint='../checkpoints/'+checkpoint,
                   ...
                   normalization='normalization.json',subset='cross7')
    if stage == 'force_pose':
...
(out/'provenance.json').write_text(json.dumps(dict(reference=reference,target=target,
    ...
    note='Recovered cross7 training normalization. Historical checkpoints do not embed preprocessing metadata; the exact global normalization for CN_300 is not recoverable from weights alone.'),indent=2))
